...
class HarmonographModule(TransformModule):
    ...
    def _load_config(self):
        ...
        # Ensure end_amp defaults exist for preset paths
        if not hasattr(self, 'end_amp1'):
            self.end_amp1 = self._getfloat('end_amp1', self.amp1)
            self.end_amp2 = self._getfloat('end_amp2', self.amp2)
            self.end_amp3 = self._getfloat('end_amp3', self.amp3)
            self.end_amp4 = self._getfloat('end_amp4', self.amp4)
    ...
    def _load_custom_config(self):
        ...
        # Drift: end amplitudes (interpolated over draw)
        self.end_amp1 = self._getfloat('end_amp1', self.amp1)
        self.end_amp2 = self._getfloat('end_amp2', self.amp2)
        self.end_amp3 = self._getfloat('end_amp3', self.amp3)
        self.end_amp4 = self._getfloat('end_amp4', self.amp4)
    ...
    def transform(self, z: complex, t: float) -> complex:
        ...
        if self.freq3 > 0:
            x3 = a3 * np.sin(self.freq3 * 2 * pi * time + self.phase3)
            if self.decay3 > 0:
                x3 *= np.exp(-self.decay3 * time)
            x += x3

        # Y component (pendulum 2 + pendulum 4)
        y = a2 * np.sin(self.freq2 * 2 * pi * time + self.phase2)
        if self.decay2 > 0:
            y *= np.exp(-self.decay2 * time)

        if self.freq4 > 0:
            y4 = a4 * np.sin(self.freq4 * 2 * pi * time + self.phase4)
            if self.decay4 > 0:
