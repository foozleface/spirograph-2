...
from spiro.pipeline import engine as pipeline_engine
from spiro.pipeline.ini import PLOT_SAMPLING, build_ini
from spiro.pipeline.registry import MODULE_DEFS, TYPE_TO_MODULE
...
# --------------------------------------------------------------------------- #
# The module table now lives in spiro.pipeline.registry, which nothing else
# imports — so the desktop app and this server describe the same modules.
...
# PLOT_SAMPLING (imported above) is what every plot is sampled at, whatever
# the preview used; see spiro.pipeline.ini for why.
...
def _build_ini(req: GenerateRequest) -> str:
    """The request's pipeline as INI text. The writing itself is
    spiro.pipeline.ini's job; this only unpacks the request model."""
    return build_ini(steps=req.steps, output=req.output, sampling=req.sampling,
                     symmetry=req.symmetry, arms=req.arms,
                     global_mods=req.global_mods, pipeline=req.pipeline)


def _run_pipeline_points(ini_text: str, target_width: float = None, target_height: float = None):
    """Run a pipeline and return (normalized point arrays, style config).

    The legacy shape: paths already fitted into a canvas whose size the fit
    itself decided. spiro.pipeline.engine keeps the drawing in its own units;
    this reproduces the old contract for the routes that still expect it.
    """
    drawing = pipeline_engine.run(ini_text, reload_generators=True)
    paths, actual_w, actual_h = pipeline_engine.normalize(
        drawing, target_width, target_height)
    cfg = dict(drawing.style, width=actual_w, height=actual_h)
    return paths, cfg
